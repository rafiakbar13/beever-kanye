...
const MyQuotes: React.FC<MyQuotesProps> = (props: MyQuotesProps) => {
    const dispatch = useDispatch();
    const [myQuote, setMyQuote] = useState<string>('');
    const myQuotes: string[] = useSelector((state: any) => state.quote.myQuotes);

    const handleMyQuote = (e: ChangeEvent<HTMLInputElement>) => {
        setMyQuote(e.target.value);
    };

    const addMyQuote = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
                ...
                <form action="" className='' onSubmit={addMyQuote}>
                    <div className='flex whitespace-nowrap gap-x-2 py-3'>
                        <input
                            ...
                            name=""
                            id=""
                            className='bg-zinc-300 w-full rounded-md px-3'
                            onChange={handleMyQuote}
                            value={myQuote}
